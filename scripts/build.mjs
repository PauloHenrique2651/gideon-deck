...
<main id="main">
<div class="page-shell">${bc ? bc.visual : ''}<section class="page-hero${page.kind === 'home' ? ' homepage-hero' : ''}">${page.kind === 'home' ? `<div class="hero-motion" aria-hidden="true"><video data-hero-video muted playsinline preload="metadata" poster="/assets/images/gideon-systems-poster.jpg"><source src="/assets/video/gideon-systems.mp4" type="video/mp4"></video></div>` : ''}<div class="hero-text"><p class="eyebrow">${esc(t(ui.serviceLabel,lang))}</p><h1>${esc(t(page.h1,lang))}</h1><p class="lead">${esc(t(page.intro,lang))}</p><div class="hero-actions"><a class="button-primary" href="${whatsapp(page,lang)}" target="_blank" rel="noopener noreferrer" data-quote data-cta-location="hero">${esc(t(page.cta || ui.talk,lang))}<span aria-hidden="true">↗</span></a>${page.kind === 'home' ? `<a class="button-secondary" href="#solutions">${esc(t(ui.explore,lang))}</a>` : ''}</div><p class="cta-note">${esc(t(ui.direct,lang))}</p></div><div class="hero-panel" aria-hidden="true"><div class="hero-panel-inner"><span>G</span><i></i></div></div></section>
